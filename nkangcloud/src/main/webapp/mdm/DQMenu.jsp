	...
	$(document).on("click","isCheck",function(){
		...
		$("#map_base svg g path").on("click",function(){
		  ...
		  function commonChart2()
		  {
			...
			$.ajax({
				...
				error:function(data)
				{
				   ...
				   loadChart2();
				 //  $("#chart2 svg g").attr("transform","translate(0,310)");
			//	   $("#chart2 svg").children().find("g").eq(2).attr("onclick","hello(this)");
		  ...
		  $("#districtSelect").on("click",function(){
			...
			type : "POST",
			dataType : "json",
			url : "getAllDistrict?userState="+$("#userState").text(),
			success : function(data) {
				  if (data) {
					  ...
					  for(var i=0;i<data.length;i++){
						...
						if(data.length%2==1)
						{
							text=text+"<div style='position:relative;width:50%;height:50px;float:left;text-align:center;'></div>"
						}
						text=text+"</div></form>";
						swal(
								{
								...
								function(inputValue){
									...
									//	if(count==10){
										 $.ajax({
											 url:'getNewChart2?userState='+$("#userState").text().trim(),
											 type:"POST",
...
<div id="countrys" style="display:none">${ countryName }</div>
 ...
 ${addressInfo} |
	...
	<div class="loader more" id="goaway"></div>
				...
				<ul class="nav pull-right top-menu" style="margin-top:-60px;float: right;">
					<li class="dropdown"><nobr>Welcome <a href="mdm/profile.jsp?UID=${ uid }" class="dropdown-toggle"
						data-toggle="dropdown"
						style="font-size: 15px; margin: 0px; padding: 5px 0px;">
							<span class="username colorBlue" id="username"> ${ userInfo.nickname }
						</span>
					</a></nobr> <br/>
					<span><a style="float: right;" href="mdm/profile.jsp?UID=${ uid }"> <img id="userImage"
...
<div id="districtSelect" style="position: absolute;width: 30px;height: 30px;top: 330px;right: 3%;display:none;">
 <img src="Jsp/PIC/growth.png" alt="" width="30px" />
</div>
  <div class="sk-circle">
...
<span id="map-tip"></span> <span id="scroll-top"><a class="scrollup"><img class="scroll-top" src="http://leshu.bj.bcebos.com/icon/upgrade.png" alt="" width="30px" /></a></span> 
...
<script>
	...
	'map_141':{
		'hover': 'SINGAPORE','upColor':'#FFCC5F','overColor':'#ffcc5f','downColor':'#477cb2','enable':true,
	},
	'map_142':{
	...
	'map_172':{
		'hover': 'UNITED STATES','upColor':'#00B287','overColor':'#ffcc5f','downColor':'#477cb2','enable':true,
	},
	'map_173':{
